...
\newcommand{\outDate}{Sept. 09, 2024}
\newcommand{\dueDate}{Sept. 23, 2024}
\newcommand{\taNames}{Meher, Haohui, Neeha}
...
\part[3] \textbf{Numerical answer:} Consider an RNN (Elman Network) that takes inputs $\xv_t \in \{0,1\}^2$, has hidden vectors $\hv_t \in \Rb^2$, and output units $y_t \in \Rb$ for all $t \in \{1,\ldots,T\}$. Assume the recurrence is given by:
    ...
    \end{align*}
    where $\text{slide}(a) = \min(1,\max(0, a))$ is the activation function. Let $W_{hh} \in \mathbb{R}^{2 \times 2}$ be defined as follows: $$W_{hh} = \begin{bmatrix}0 & 1 \\ 1 & 0\end{bmatrix}$$
    Define parameters 
    % $W_{hh}\in \Rb^{2 \times 2}, W_{hx} \in \Rb^{2 \times 2},
...
\begin{subparts}
    ...
    \begin{align*}
        ...
        g_t &= \sigma(W_{gg}  g_{t+1} + W_{gx}  x_t + b_g), \quad \forall t \in \{1,\dots, T\} \\
        h_t &= \sigma(W_{hf}  f_{t} + W_{hg}  g_t + b_g), \quad \forall t \in \{1,\dots, T\} 
    \end{align*}
    (Notice that $f_t$ builds up context from the left, $g_t$ builds up context from the right, and $h_t$ combines the two.)
    Can we define an autoregressive language model of the form $p(\xv_{1:T}) = \prod_{t=1}^T p(x_t \mid h_{t-1})$? If so, define the probability distribution $p(x_t \mid \text{BiRNN}(\xv_{1:t-1}))$. If not, why not? Assume that \textbf{no} weight matrix can be set to all zeros.  


    \begin{answer_box}[title=,height=4cm,width=15cm]        
    \end{answer_box}

    \subpart[2] \textbf{Short answer:} Suppose $\text{BiRNN}(\xv_{1:t-1})$ computes a bidirectional RNN on the subsequence $\xv_{1:t-1}$ and then returns $h_{t-1}$. Can we define an autoregressive language model of the form $p(\xv_{1:T}) = \prod_{t=1}^T p(x_t \mid \text{BiRNN}(\xv_{1:t-1})$? If so, define the probability distribution $p(x_t \mid \text{BiRNN}(\xv_{1:t-1}))$. If not, why not?
...
\begin{subparts}
    ...
    \end{align*}
    where $\Wv_s \in \Rb^{d_k \times d_k}$ is a parameter matrix.
    %
...
\part The simplest way to define sliding window attention is by setting the causal mask $\Mv$ to only include a window of $\frac{1}{2}w+1$ tokens, with the rightmost window element being the current token (i.e. on the diagonal). Then our attention computation is:
\begin{align}
...
\clearpage
    ...
    \part \textbf{Pseudocode:} Fill in the blanks with pseudocode/math to create a function that takes in the queries, keys, and values and the window size $w$ and computes the $\Xv'$ of Equation \eqref{eq:slidingwindow}:
    $$\textsc{SlidingWindowAttention}(\Qv, \Kv, \Vv, w)$$
    Your pseudocode/math must have lower asymptotic computational than the naive matrix multiplication approach described above. The function $\text{softmax}(\xv)$ applies softmax to a vector $\xv$, and the function $\text{tensor}(\cdot)$ can be used to contruct vectors, matrices, tensors of arbitrary shape. In the pseudocode below, $X_i$ means indexing into the vector / matrix $X$ at index $i$.
...
def SlidingWindowAttention(@$Q, K, V, w$@):
    ...
    @$w'$@ =  @\underline{~~\textbf{(1)}~~}@ 
    for @$j$@ in @$1$@ to @$N$@:
        @$a$@ = tensor(@\underline{~~\textbf{(2)}~~}@)
        for @$i$@ in @\underline{~~\textbf{(3)}~~}@:
    ...
    \subpart[1] \textbf{Short answer:} What is the time complexity of your pseudocode in terms of $N$ and $w$?
...
\part[4] Plot the training loss for both your RoPE implementation and the vanilla minGPT over \textbf{1200 total training iterations} on the \textbf{same plot}: 600 iterations with a \textbf{sequence length of 16}, followed by 600 iterations with a \textbf{sequence length of 256}. 
...
\part[2] Provide a sample from your RoPE model after \textbf{600 iterations} of training with a \textbf{sequence length of 16}. Condition the sample on the first line of your favorite Shakespeare play. 

[Expected runtime on Colab T4: approximately 3 minutes]
\begin{answer_box}[title=,height=9cm, width=15cm]
\end{answer_box}

\part[2] Provide a sample from your RoPE model after \textbf{1200 iterations}, consisting of 600 iterations with a \textbf{sequence length of 16}, followed by 600 iterations with a \textbf{sequence length of 256}. Condition the sample on the first line of your favorite Shakespeare play. 
...
\part[4] Plot the \textbf{average memory consumption} in MB per iteration across \{1, 2, 3, 6\} number of key heads with a \textbf{sequence length of 16} over \textbf{200 iterations}.
